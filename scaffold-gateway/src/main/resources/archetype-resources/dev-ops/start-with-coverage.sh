#!/usr/bin/env bash
# 带 JaCoCo Agent 启动本网关，用于分布式 E2E 覆盖率采集。
#
# Agent 以 tcpserver 模式在本机端口监听，由 coverage-controller 主动连接 dump，
# 因此网关无需感知控制器地址，也不要求控制器先启动。
#
#   COVERAGE_AGENT_NAME        Agent 名称，需与控制器 coverage.agents[].name 一致（默认取工程目录名）
#   COVERAGE_AGENT_PORT        Agent 监听端口（默认 6300）
...
COVERAGE_AGENT_NAME="${COVERAGE_AGENT_NAME:-$(basename "${PROJECT_DIR}")}"
